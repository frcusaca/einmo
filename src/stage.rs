//! The three-stage lifecycle and stage-directory operations (FOOP-92 §3;
//! `EIMP-7` §S.2a narrowed this from four stages to three — `flagged/` is
//! no longer a stage of its own, but a sink nested inside each of these
//! three, addressed via `storage::ArtifactLocation::Flagged(Stage)`).

use std::path::{Path, PathBuf};

use crate::error::{EinmoError, Result};

/// The default name of a stage's nested flagged sink (`<stage>/flagged/`,
/// `EIMP-7` §S.2a) — the one place this literal is defined, so
/// [`crate::config::StageDirs::default`] and [`crate::corpus_signer`] (which
/// has no [`crate::config::TestConfig`] to ask, and already assumes default
/// stage-directory names) never drift apart on it.
pub(crate) const DEFAULT_FLAGGED_DIR_NAME: &str = "flagged";

/// One of the four promotion stages. Each maps to a directory that mirrors
/// the `input/` tree.
///
/// Declaration order **is** lifecycle order: the derived `Ord` is relied on
/// (`EIMP-01` §S.1), so a new stage must be declared at its position in the
/// chain, not appended.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum Stage {
    /// Written by `einmo generate` — the work file (`EIMP-01` §S.1). Carries
    /// `compiled` + `configured` + `stage:generated` stamps.
    ///
    /// A stage in every respect the other three are: signed artifacts,
    /// verify-on-inspect, a nested flagged sink, promotion into the next
    /// stage. It differs in exactly two ways, and neither is visible to this
    /// type: its directory is **not committed** (`.gitignore`), and it exists
    /// to be compared against [`Stage::Output`].
    Generated,
    /// Promoted from `generated` — the committed baseline (`EIMP-01` §S.3).
    /// Adds a `stage:output` stamp, on the deliberately weak claim that the
    /// run completed and the output is *reasonable*: a sanity check, not a
    /// semantic or stylistic review.
    Output,
    /// Reviewed — promoted from `output` by an AI agent or human. Adds a
    /// `stage:checked` stamp.
    Checked,
    /// Promoted from `checked` (or `output`) by a human with a passphrase.
    /// Adds a `stage:verified` stamp.
    Verified,
}

impl Stage {
    /// All four stages, in lifecycle order.
    pub const ALL: [Stage; 4] = [
        Stage::Generated,
        Stage::Output,
        Stage::Checked,
        Stage::Verified,
    ];

    /// The default directory name for this stage.
    #[must_use]
    pub fn dir_name(self) -> &'static str {
        match self {
            Stage::Generated => "generated",
            Stage::Output => "output",
            Stage::Checked => "checked",
            Stage::Verified => "verified",
        }
    }

    /// The `stage:<name>` stamp key this stage appends on promotion.
    #[must_use]
    pub fn stamp_key(self) -> &'static str {
        match self {
            Stage::Generated => "stage:generated",
            Stage::Output => "stage:output",
            Stage::Checked => "stage:checked",
            Stage::Verified => "stage:verified",
        }
    }

    /// Parse a stage from its lowercase directory name. `"flagged"` is
    /// deliberately not a valid stage name (`EIMP-7` §S.2a): flagged
    /// content is addressed as `ArtifactLocation::Flagged(Stage)`, not as
    /// a bare stage of its own.
    ///
    /// # Errors
    ///
    /// Returns [`EinmoError::InvalidStageName`] for any unrecognized name.
    pub fn parse(name: &str) -> Result<Self> {
        match name {
            "generated" => Ok(Stage::Generated),
            "output" => Ok(Stage::Output),
            "checked" => Ok(Stage::Checked),
            "verified" => Ok(Stage::Verified),
            other => Err(EinmoError::InvalidStageName(other.to_string())),
        }
    }
}

impl std::fmt::Display for Stage {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(self.dir_name())
    }
}

impl<'de> serde::Deserialize<'de> for Stage {
    /// Routes through [`Stage::parse`] so a request path segment names a
    /// stage exactly the same way any other `Stage` construction does — an
    /// invalid name is a deserialization error (axum's `Path<Stage>`
    /// extractor turns this into a `400` before handler code runs,
    /// mirroring [`EinmoId`]'s own `Deserialize` impl further down in
    /// this file).
    fn deserialize<D: serde::Deserializer<'de>>(
        deserializer: D,
    ) -> std::result::Result<Self, D::Error> {
        let s = String::deserialize(deserializer)?;
        Stage::parse(&s).map_err(serde::de::Error::custom)
    }
}

/// Validate a stage or stage-directory name against `[A-Za-z0-9_-]+`.
///
/// # Errors
///
/// Returns [`EinmoError::InvalidStageName`] if the name is empty or contains
/// any character outside the allowed set.
pub fn validate_stage_name(name: &str) -> Result<()> {
    if !name.is_empty()
        && name
            .chars()
            .all(|c| c.is_ascii_alphanumeric() || c == '_' || c == '-')
    {
        Ok(())
    } else {
        Err(EinmoError::InvalidStageName(name.to_string()))
    }
}

/// Map an input-relative path to its mirror-relative `.einmo` path.
///
/// `stage1/section3/specific.foo` → `stage1/section3/specific.foo.einmo`.
/// Discovery is extension-agnostic — any file under `input/` is a test
/// trigger, so `.foo`, `.py`, and `.js` all map by appending `.einmo`.
#[must_use]
pub fn mirror_input_path(input_rel: &Path) -> PathBuf {
    let mut os = input_rel.as_os_str().to_os_string();
    os.push(".einmo");
    PathBuf::from(os)
}

/// A validated identifier for one reviewable case (EIMP-2 §0, einmo repo).
///
/// Formalizes the identity [`mirror_input_path`] already computes: the
/// input-relative path with its input extension preserved (so
/// same-stem-different-extension inputs, e.g. `x.foo` and `x.js`, never
/// collide), stored WITHOUT the trailing `.einmo` mirror suffix — e.g.
/// `foop/23/comprehensive.foo` for an input at
/// `input/foop/23/comprehensive.foo`. Stable across stages: the same id
/// names the input and every stage artifact
/// (`output/<id>.einmo`, `checked/<id>.einmo`, …).
#[derive(Debug, Clone, PartialEq, Eq, Hash, PartialOrd, Ord)]
pub struct EinmoId(String);

impl EinmoId {
    /// Construct from an input-relative path (e.g. one returned by
    /// [`walk_input_tree`]).
    ///
    /// # Errors
    ///
    /// Returns [`EinmoError::InvalidId`] if the path fails validation (see
    /// [`validate_id_str`]).
    pub fn from_input_rel(input_rel: &Path) -> Result<Self> {
        let s = input_rel.to_string_lossy();
        validate_id_str(&s)?;
        // Normalize to forward slashes regardless of host path separator,
        // so the id is stable across platforms and safe as a URL segment.
        Ok(EinmoId(s.replace('\\', "/")))
    }

    /// Derive an id from any stage artifact's path, given that stage's root
    /// directory. Strips the stage root and the trailing `.einmo` suffix,
    /// recovering the same id [`EinmoId::from_input_rel`] would have
    /// produced for the same case. Lets a case be identified even when its
    /// `input/` file is missing (e.g. deleted after being promoted).
    ///
    /// # Errors
    ///
    /// Returns [`EinmoError::InvalidId`] if `artifact_path` is not under
    /// `stage_root`, does not end in `.einmo`, or the recovered id fails
    /// validation.
    pub fn from_stage_artifact_path(stage_root: &Path, artifact_path: &Path) -> Result<Self> {
        let rel = artifact_path.strip_prefix(stage_root).map_err(|_| {
            EinmoError::InvalidId(format!(
                "{} is not under stage root {}",
                artifact_path.display(),
                stage_root.display()
            ))
        })?;
        let rel_str = rel.to_string_lossy();
        let without_suffix = rel_str.strip_suffix(".einmo").ok_or_else(|| {
            EinmoError::InvalidId(format!(
                "{} does not end in .einmo",
                artifact_path.display()
            ))
        })?;
        validate_id_str(without_suffix)?;
        Ok(EinmoId(without_suffix.replace('\\', "/")))
    }

    /// Construct this case's artifact path in a given stage, rooted at the
    /// suite directory — e.g. `output/foop/23/comprehensive.foo.einmo`.
    #[must_use]
    pub fn to_stage_path(&self, suite_root: &Path, stage: Stage) -> PathBuf {
        suite_root
            .join(stage.dir_name())
            .join(mirror_input_path(Path::new(&self.0)))
    }

    /// The validated id as a `&str` (forward-slash separated, no `.einmo`
    /// suffix) — used for URL path segments (percent-encode at the call
    /// site; this method does not encode) and log lines.
    #[must_use]
    pub fn as_str(&self) -> &str {
        &self.0
    }
}

impl std::fmt::Display for EinmoId {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(&self.0)
    }
}

impl TryFrom<&str> for EinmoId {
    type Error = EinmoError;

    /// Parses an incoming (already percent-decoded) URL path segment or any
    /// other caller-supplied string, applying the same validation as
    /// [`EinmoId::from_input_rel`].
    fn try_from(s: &str) -> Result<Self> {
        validate_id_str(s)?;
        Ok(EinmoId(s.replace('\\', "/")))
    }
}

impl serde::Serialize for EinmoId {
    fn serialize<S: serde::Serializer>(
        &self,
        serializer: S,
    ) -> std::result::Result<S::Ok, S::Error> {
        serializer.serialize_str(&self.0)
    }
}

impl<'de> serde::Deserialize<'de> for EinmoId {
    /// Routes through [`EinmoId::try_from`] so a request path segment or a
    /// JSON body field that names a case is validated identically to any
    /// other `EinmoId` construction path — an invalid id is a
    /// deserialization error (axum's `Path<EinmoId>`/`Json<_>` extractors
    /// turn this into a `400` before handler code runs, EIMP-2 §3a).
    fn deserialize<D: serde::Deserializer<'de>>(
        deserializer: D,
    ) -> std::result::Result<Self, D::Error> {
        let s = String::deserialize(deserializer)?;
        EinmoId::try_from(s.as_str()).map_err(serde::de::Error::custom)
    }
}

/// Validate an `EinmoId` candidate string: non-empty, no NUL byte, no `..`
/// path component, not absolute (no leading `/` or `\`, no drive letter).
///
/// # Errors
///
/// Returns [`EinmoError::InvalidId`] on any violation.
fn validate_id_str(s: &str) -> Result<()> {
    if s.is_empty() {
        return Err(EinmoError::InvalidId("empty id".to_string()));
    }
    if s.contains('\0') {
        return Err(EinmoError::InvalidId(s.to_string()));
    }
    if s.starts_with('/') || s.starts_with('\\') {
        return Err(EinmoError::InvalidId(s.to_string()));
    }
    for component in s.split(['/', '\\']) {
        if component.is_empty() || component == ".." {
            return Err(EinmoError::InvalidId(s.to_string()));
        }
    }
    Ok(())
}

/// Recursively discover every file under `input_dir`, returning
/// input-relative paths in sorted (deterministic) order.
///
/// `depth_limit` caps the recursion depth (catches symlink cycles); pass
/// [`MAX_WALK_DEPTH`] for the default.
///
/// # Errors
///
/// Returns [`EinmoError::Io`] if the tree cannot be walked.
pub fn walk_input_tree(input_dir: &Path, depth_limit: usize) -> Result<Vec<PathBuf>> {
    Ok(walk_input_tree_reporting(input_dir, depth_limit)?.0)
}

/// `true` if `rel`'s first path component is `flagged_dir_name` — i.e.
/// `rel` lives inside a stage's nested flagged sink (`EIMP-7` §S.2a)
/// rather than being one of that stage's own artifacts.
///
/// Every caller that walks a STAGE directory expecting only its own
/// artifacts (orphan detection, suite scanning, corpus signing, storage
/// listing) must filter its result through this, or a flagged artifact
/// gets silently treated as a regular one. Callers that walk `input/`
/// must NOT apply this filter: a user-authored section legitimately
/// named `flagged` under `input/` is not einmo's flagged sink and must
/// not be excluded from discovery.
#[must_use]
pub fn is_in_flagged_sink(rel: &Path, flagged_dir_name: &str) -> bool {
    rel.components().next()
        == Some(std::path::Component::Normal(std::ffi::OsStr::new(
            flagged_dir_name,
        )))
}

/// As [`walk_input_tree`], but also returns the **extraneous** entries it
/// refused to treat as inputs (dot-prefixed files and directories).
///
/// Discovery skips them so an open editor cannot inject phantom tests; this
/// variant hands them back so the caller can *report* them. Einmo never
/// silently ignores a file in its own tree — a suite whose shape is wrong is a
/// failure, not a detail.
///
/// # Errors
///
/// Returns [`EinmoError::Io`] if the tree cannot be walked.
pub fn walk_input_tree_reporting(
    input_dir: &Path,
    depth_limit: usize,
) -> Result<(Vec<PathBuf>, Vec<PathBuf>)> {
    let mut found = Vec::new();
    let mut extraneous = Vec::new();
    walk_dir_depth(
        input_dir,
        input_dir,
        &mut found,
        &mut extraneous,
        0,
        depth_limit,
    )?;
    found.sort();
    extraneous.sort();
    Ok((found, extraneous))
}

/// Maximum recursion depth for the directory walk. Catches pathologically deep
/// trees as well as symlink cycles (a symlink cycle would otherwise recurse
/// forever). This is the default used by [`TestConfig::new`](crate::config::TestConfig::new).
pub(crate) const MAX_WALK_DEPTH: usize = 64;

fn walk_dir_depth(
    root: &Path,
    dir: &Path,
    out: &mut Vec<PathBuf>,
    extraneous: &mut Vec<PathBuf>,
    depth: usize,
    depth_limit: usize,
) -> Result<()> {
    if depth > depth_limit {
        return Err(EinmoError::Io {
            path: dir.to_path_buf(),
            source: std::io::Error::other(format!(
                "directory walk exceeded max depth {depth_limit} (possible symlink cycle)"
            )),
        });
    }
    if !dir.exists() {
        return Ok(());
    }
    let entries = std::fs::read_dir(dir).map_err(|e| EinmoError::io(dir, e))?;
    for entry in entries {
        let entry = entry.map_err(|e| EinmoError::io(dir, e))?;
        let path = entry.path();
        // Dot-prefixed entries (editor swap files, `.git`, `.DS_Store`) are
        // never test inputs: a test input is a file someone deliberately named.
        // They are skipped for *discovery* — an open editor must not inject
        // phantom tests — but recorded as extraneous so the caller can report
        // them. Einmo never silently ignores a file in its own tree.
        if entry
            .file_name()
            .to_str()
            .is_some_and(|n| n.starts_with('.'))
        {
            if let Ok(rel) = path.strip_prefix(root) {
                extraneous.push(rel.to_path_buf());
            }
            continue;
        }
        let file_type = entry.file_type().map_err(|e| EinmoError::io(&path, e))?;
        if file_type.is_symlink() {
            // Follow the symlink to determine the target type. A broken
            // symlink (target missing → NotFound) is skipped silently — it is
            // not a valid test input. Other metadata failures (e.g. ELOOP
            // from a symlink cycle) are propagated as errors.
            let metadata = match std::fs::metadata(&path) {
                Ok(m) => m,
                Err(e) if e.kind() == std::io::ErrorKind::NotFound => continue,
                Err(e) => return Err(EinmoError::io(&path, e)),
            };
            if metadata.is_dir() {
                walk_dir_depth(root, &path, out, extraneous, depth + 1, depth_limit)?;
            } else if metadata.is_file() {
                let rel = path
                    .strip_prefix(root)
                    .map_err(|_| EinmoError::Io {
                        path: path.clone(),
                        source: std::io::Error::other("path not under input root"),
                    })?
                    .to_path_buf();
                out.push(rel);
            }
        } else if file_type.is_dir() {
            walk_dir_depth(root, &path, out, extraneous, depth + 1, depth_limit)?;
        } else if file_type.is_file() {
            let rel = path
                .strip_prefix(root)
                .map_err(|_| EinmoError::Io {
                    path: path.clone(),
                    source: std::io::Error::other("path not under input root"),
                })?
                .to_path_buf();
            out.push(rel);
        }
    }
    Ok(())
}

/// Ensure the parent directory of `path` exists (creating the mirrored subtree
/// on demand).
///
/// # Errors
///
/// Returns [`EinmoError::Io`] if directory creation fails.
pub fn ensure_parent_dir(path: &Path) -> Result<()> {
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).map_err(|e| EinmoError::io(parent, e))?;
    }
    Ok(())
}

#[cfg(test)]
mod tests {
    use super::*;
    use std::fs;

    #[test]
    fn stage_names_and_stamp_keys() {
        assert_eq!(Stage::Output.dir_name(), "output");
        assert_eq!(Stage::Verified.stamp_key(), "stage:verified");
        assert_eq!(Stage::parse("checked").unwrap(), Stage::Checked);
        assert!(Stage::parse("bogus").is_err());
    }

    /// `EIMP-01` §S.1 — the generation stage is uniform with the other three:
    /// a directory name, a stamp key, and a name that parses.
    #[test]
    fn generated_stage_names_and_stamp_key() {
        assert_eq!(Stage::Generated.dir_name(), "generated");
        assert_eq!(Stage::Generated.stamp_key(), "stage:generated");
        assert_eq!(Stage::parse("generated").unwrap(), Stage::Generated);
    }

    /// `EIMP-01` §S.1 — `ALL` is the lifecycle, in order. Written as an exact
    /// equality rather than a length check so that adding a stage without
    /// placing it correctly fails here rather than somewhere subtler.
    #[test]
    fn all_stages_are_the_lifecycle_in_order() {
        assert_eq!(
            Stage::ALL,
            [
                Stage::Generated,
                Stage::Output,
                Stage::Checked,
                Stage::Verified
            ]
        );
    }

    /// `EIMP-01` §S.1 — `Generated` is declared FIRST so the derived `Ord`
    /// keeps lifecycle order. A stage added in the wrong declaration position
    /// would still compile and still round-trip by name; only this catches it.
    #[test]
    fn stage_ordering_follows_the_lifecycle() {
        assert!(Stage::Generated < Stage::Output);
        assert!(Stage::Output < Stage::Checked);
        assert!(Stage::Checked < Stage::Verified);
    }

    /// `EIMP-01` §S.1 — `generated` needs no relaxation of the stage-name
    /// rule (the `output.gen` spelling considered during design would have;
    /// this pins that the chosen name does not).
    #[test]
    fn generated_is_a_valid_stage_name_unchanged() {
        assert!(validate_stage_name(Stage::Generated.dir_name()).is_ok());
    }

    #[test]
    fn validate_stage_name_rules() {
        assert!(validate_stage_name("output").is_ok());
        assert!(validate_stage_name("output-2").is_ok());
        assert!(validate_stage_name("out_put").is_ok());
        assert!(validate_stage_name("").is_err());
        assert!(validate_stage_name("out>put").is_err());
        assert!(validate_stage_name("out put").is_err());
    }

    #[test]
    fn einmo_id_round_trips_through_stage_path() {
        let id = EinmoId::from_input_rel(Path::new("foop/23/comprehensive.foo")).unwrap();
        assert_eq!(id.as_str(), "foop/23/comprehensive.foo");
        for stage in Stage::ALL {
            let suite_root = Path::new("/suite");
            let stage_path = id.to_stage_path(suite_root, stage);
            let expected = suite_root
                .join(stage.dir_name())
                .join("foop/23/comprehensive.foo.einmo");
            assert_eq!(stage_path, expected, "stage {stage:?}");
            let recovered =
                EinmoId::from_stage_artifact_path(&suite_root.join(stage.dir_name()), &stage_path)
                    .unwrap();
            assert_eq!(recovered, id, "recovered id for stage {stage:?}");
        }
    }

    #[test]
    fn einmo_id_from_stage_artifact_path_recovers_flat_id() {
        let stage_root = Path::new("/suite/checked");
        let artifact = stage_root.join("simple.foo.einmo");
        let id = EinmoId::from_stage_artifact_path(stage_root, &artifact).unwrap();
        assert_eq!(id.as_str(), "simple.foo");
    }

    #[test]
    fn einmo_id_rejects_traversal_and_absolute_and_nul() {
        assert!(EinmoId::from_input_rel(Path::new("../escape.foo")).is_err());
        assert!(EinmoId::from_input_rel(Path::new("a/../b.foo")).is_err());
        assert!(EinmoId::from_input_rel(Path::new("/abs.foo")).is_err());
        assert!(EinmoId::try_from("../escape.foo").is_err());
        assert!(EinmoId::try_from("a/b\0c.foo").is_err());
        assert!(EinmoId::try_from("").is_err());
    }

    #[test]
    fn einmo_id_try_from_str_matches_from_input_rel() {
        let from_path = EinmoId::from_input_rel(Path::new("nested/name_binding.js")).unwrap();
        let from_str = EinmoId::try_from("nested/name_binding.js").unwrap();
        assert_eq!(from_path, from_str);
    }

    #[test]
    fn einmo_id_distinguishes_same_stem_different_extension() {
        let foo = EinmoId::from_input_rel(Path::new("x.foo")).unwrap();
        let js = EinmoId::from_input_rel(Path::new("x.js")).unwrap();
        assert_ne!(foo, js);
        assert_ne!(
            foo.to_stage_path(Path::new("/s"), Stage::Output),
            js.to_stage_path(Path::new("/s"), Stage::Output)
        );
    }

    #[test]
    fn einmo_id_display_matches_as_str() {
        let id = EinmoId::from_input_rel(Path::new("a/b.foo")).unwrap();
        assert_eq!(id.to_string(), id.as_str());
    }

    #[test]
    fn flat_input_maps_to_flat_mirror() {
        assert_eq!(
            mirror_input_path(Path::new("simple.foo")),
            PathBuf::from("simple.foo.einmo")
        );
    }

    #[test]
    fn hierarchical_input_maps_to_mirrored_path() {
        assert_eq!(
            mirror_input_path(Path::new("stage1/section3/specific.foo")),
            PathBuf::from("stage1/section3/specific.foo.einmo")
        );
    }

    #[test]
    fn non_foo_extensions_discovered() {
        let tmp = tempfile::tempdir().unwrap();
        let input = tmp.path().join("input");
        fs::create_dir_all(input.join("nested")).unwrap();
        fs::write(input.join("a.py"), "1+2").unwrap();
        fs::write(input.join("nested/b.js"), "1+2").unwrap();
        fs::write(input.join("nested/c.foo"), "{3;}").unwrap();

        let found = walk_input_tree(&input, MAX_WALK_DEPTH).unwrap();
        assert_eq!(
            found,
            vec![
                PathBuf::from("a.py"),
                PathBuf::from("nested/b.js"),
                PathBuf::from("nested/c.foo"),
            ]
        );
    }

    #[test]
    fn same_basename_different_branches_coexist() {
        let a = mirror_input_path(Path::new("x/test.foo"));
        let b = mirror_input_path(Path::new("y/test.foo"));
        assert_ne!(a, b);
        assert_eq!(a, PathBuf::from("x/test.foo.einmo"));
        assert_eq!(b, PathBuf::from("y/test.foo.einmo"));
    }

    #[cfg(unix)]
    #[test]
    fn symlinked_file_discovered() {
        use std::os::unix::fs::symlink;
        let tmp = tempfile::tempdir().unwrap();
        let input = tmp.path().join("input");
        fs::create_dir_all(&input).unwrap();
        fs::write(input.join("real.py"), "1+2").unwrap();
        symlink(input.join("real.py"), input.join("link.py")).unwrap();

        let found = walk_input_tree(&input, MAX_WALK_DEPTH).unwrap();
        let names: Vec<String> = found
            .iter()
            .map(|p| p.file_name().unwrap().to_string_lossy().into_owned())
            .collect();
        assert!(names.contains(&"real.py".to_string()));
        assert!(names.contains(&"link.py".to_string()));
    }

    #[cfg(unix)]
    #[test]
    fn symlinked_dir_recursed() {
        use std::os::unix::fs::symlink;
        let tmp = tempfile::tempdir().unwrap();
        let input = tmp.path().join("input");
        let real_dir = input.join("real_dir");
        fs::create_dir_all(&real_dir).unwrap();
        fs::write(real_dir.join("inside.foo"), "{3;}").unwrap();
        symlink(&real_dir, input.join("link_dir")).unwrap();

        let found = walk_input_tree(&input, MAX_WALK_DEPTH).unwrap();
        assert!(
            found
                .iter()
                .any(|p| p == &PathBuf::from("link_dir/inside.foo")),
            "file inside symlinked dir not discovered: {found:?}"
        );
    }

    #[cfg(unix)]
    /// An editor swap file (or any dot-prefixed entry) is not a test input:
    /// an open vim session must not inject phantom tests into the suite.
    #[test]
    fn walk_skips_hidden_entries() {
        let tmp = tempfile::tempdir().unwrap();
        let input = tmp.path().join("input");
        std::fs::create_dir_all(input.join("foop/13")).unwrap();
        std::fs::write(input.join("foop/13/comprehensive.foo"), "{1;}").unwrap();
        std::fs::write(input.join("foop/13/.comprehensive.foo.swp"), "vim").unwrap();
        std::fs::create_dir_all(input.join(".git")).unwrap();
        std::fs::write(input.join(".git/config"), "x").unwrap();

        let found = walk_input_tree(&input, MAX_WALK_DEPTH).unwrap();
        assert_eq!(
            found,
            vec![PathBuf::from("foop/13/comprehensive.foo")],
            "hidden files and dot-directories must not be discovered as inputs"
        );
    }

    #[test]
    fn walk_depth_limit_prevents_infinite_symlink_cycle() {
        use std::os::unix::fs::symlink;
        let tmp = tempfile::tempdir().unwrap();
        let input = tmp.path().join("input");
        fs::create_dir_all(&input).unwrap();
        // A symlink cycle: a -> b -> a. Following either link hits ELOOP,
        // which the walker propagates as an error rather than hanging.
        symlink(input.join("b"), input.join("a")).unwrap();
        symlink(input.join("a"), input.join("b")).unwrap();

        let result = walk_input_tree(&input, MAX_WALK_DEPTH);
        assert!(
            result.is_err(),
            "expected walk of symlink cycle to error, got {result:?}"
        );
    }
}
