...
task(
  ...
  prompt="Find every `$X.unwrap()` in src/. Report the
...
| `scope` | no | `--scope` | sub-path to restrict the query to, e.g. `src` |
...
```
action: search   lang: rust   scope: src
pattern: pub fn $NAME($$$ARGS) -> Result<$T, $E> { $$$ }
```

```
src/lib.rs:42:pub fn verify_bytes(bytes: &[u8]) -> Result<VerificationResult, EinmoError> {
src/lib.rs:46:pub fn from_file(path: &Path) -> Result<EinmoFile, EinmoError> {
```
...
Scoped to `src/`, this returns warnings with file, line and a source excerpt.
...
**Changes**: Imported from foolish workspace, adapted for einmo. Updated example
crate names and scope references from foolish-core/foolish-parser to src/.
