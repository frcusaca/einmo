...
This is a three-directory cargo workspace (`einmo`, `einmo-tools`, `zweimomo`).
The tool works on exactly one crate per call.
...
## AVAILABLE CRATES
- einmo                  XX files    XXXK  (./)
- einmo-tools            XX files    XXXK  (einmo-tools/)
- zweimomo               XX files    XXXK  (zweimomo/)
```
...
```bash
bun .opencode/tools/bundle_repo_ctx.ts --crate einmo
```
...
bun .opencode/tools/ast_grep_engine.js --action scan --lang rust \
    --yaml-rule-file /tmp/rule.yaml --scope src
```
...
**Changes**: Imported from foolish workspace, adapted for einmo's three-crate
workspace (einmo, einmo-tools, zweimomo). Updated example crate names and scope
references.
