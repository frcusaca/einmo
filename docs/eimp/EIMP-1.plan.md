...
- [x] Verify all work is committed on `jia` and all tests pass
      ...
      **Update (2026-07-31, box checked)** — `agent/1-0` merged to `jia`
      via PR #1 (`1e81f9b`). Re-ran the full suite on `jia` post-merge:
      **356 tests, 0 failed** (318 + 31 + 4 + 3, identical composition to
      the pre-merge count — no regressions), `cargo fmt --check` /
      `cargo clippy --all-targets -- -D warnings` both clean. The
      branch-vs-`jia` blocker this box was waiting on is resolved. P1
      remains open by design (architectural discussion, not a fix-it
