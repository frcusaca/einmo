...
# experimental_reviewer.sh — the poor foolisher's einmo review loop (FOOP-64).
#
...
# ── USAGE ─────────────────────────────────────────────────────────────────
#
#     ./experimental_reviewer.sh [-D] [-f] [-s] [-n] [-e EINMO] <suite-dir> [name-filter]
#
...
# shared host, prefer a private location: set EXPERIMENTAL_REVIEWER_DIR to a directory of
# your own (an encrypted or tmpfs-backed path rather than shared /tmp); it is
# still forced to 700.
#
# Examples:
#     ./experimental_reviewer.sh foolish-ubca/einmo_suite               # what needs a look
#     ./experimental_reviewer.sh -D foolish-ubca/einmo_suite            # everything
#     ./experimental_reviewer.sh -s foolish-ubca/einmo_suite foop/23    # shuffle, one FOOP
#     ./experimental_reviewer.sh -n foolish-ubca/einmo_suite            # trace, no editor
#
...
# for a user-supplied $EXPERIMENTAL_REVIEWER_DIR that may predate our umask or arrive
...
# rather than shared /tmp), set EXPERIMENTAL_REVIEWER_DIR — it is still forced private.
umask 077
...
harden_dir() {  # force owner-only on the dir AND its contents; refuse if it fails
    ...
    if [[ "$mode" != "700" ]]; then
        echo "experimental_reviewer: refusing to use $1 — could not secure it to mode 700 (got $mode)." >&2
    ...
    if [[ -n "$(find "$1" \( -perm /0077 \) -print -quit 2>/dev/null)" ]]; then
        echo "experimental_reviewer: refusing to use $1 — something under it is group/other-accessible." >&2
        exit 1
    fi
}
...
# a directory we did not make. If you set EXPERIMENTAL_REVIEWER_DIR, our subdir is created
...
if [[ -n "${EXPERIMENTAL_REVIEWER_DIR:-}" ]]; then
    mkdir -p "$EXPERIMENTAL_REVIEWER_DIR"
    harden_dir "$EXPERIMENTAL_REVIEWER_DIR"
    TMP="$(mktemp -d "$EXPERIMENTAL_REVIEWER_DIR/experimental_reviewer.XXXXXX")"
else
    TMP="$(mktemp -d -t experimental_reviewer.XXXXXX)"
fi
harden_dir "$TMP"

# Exiting experimental_reviewer removes OUR scratch subdir — nothing survives the run.
...
echo "experimental_reviewer: ${#rows[@]} test(s) in $SUITE${FILTER:+ (filter: $FILTER)}"
...
while (( i < ${#rows[@]} )); do
    ...
    status_text='experimental_reviewer · \c/\C checked · \v/\V verified · \d diff here · \D diff all · ]c/[c jump · :qa done · :cq abort · \I for bigger info window'
...
function! PoorEinmoVerb(target, verb, leave, autonext)
  if a:target < 2
    echo 'experimental_reviewer: no stage artifact to act on'
    return
  endif
...
function! PoorEinmoMark(target, verb)
  ...
  call setline(1, a:verb)
  write
  echo 'experimental_reviewer: marked ' . (a:target == 4 ? 'checked' : 'verified') . ' as ' . a:verb . ' — :qa to finish'
endfunction
...
experimental_reviewer auto-flags notes (mv to <stage>/flagged/) on settle. Promotions and
...
FINEPRINT
    ...
    if ! vim "${review_opts[@]}" "$instr" </dev/tty >/dev/tty; then
        echo
        echo "experimental_reviewer: aborted at $test_path."
        break 2
    fi
            ...
            while :; do
                ...
                case "${ans,,}" in
                    ...
                    u*|b*)
                        ...
                        jump=$((i - 1))
                        break ;;
                    q*) echo "experimental_reviewer: stopped."; break 3 ;;
                    *)  break ;;
                esac
            done
        fi
...
echo
echo "══ experimental_reviewer results ══════════════════════════════════════════════"
...
# point is that your note reliably LANDS in flagged/. So experimental_reviewer runs each
...
# The PROMOTE gate. experimental_reviewer can RUN promotions for you, but only after you
...
if ! { (( have_promotions )) && [[ "${ans:-}" == "PROMOTE" ]]; }; then
    echo
    echo "experimental_reviewer did not change the corpus: run the commands above to act on your review."
fi
